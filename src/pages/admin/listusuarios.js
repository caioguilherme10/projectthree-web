...
import { Flex, Box, Spacer, Text, Divider, Button, Avatar, Heading, Select, Skeleton } from '@chakra-ui/react'
...
const Listusuarios = () => {
    ...
    let array = [
        {id: 1, email:'caio@gmail.com', nome:'caio', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
        {id: 2, email:'jose@gmail.com', nome:'jose', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
        {id: 3, email:'carlos@gmail.com', nome:'carlos', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
        {id: 4, email:'maria@gmail.com', nome:'maria', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
        {id: 5, email:'jose@gmail.com', nome:'jose', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
        {id: 6, email:'carlos@gmail.com', nome:'carlos', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
        {id: 7, email:'maria@gmail.com', nome:'maria', senha: '', status: '', tipo: '', createdAt: '', updatedAt: ''},
    ...
    /*if(!loading){
        ...
        let a = data.users.length;
        const num = () => {
            let x = a;
            let y = 7;
            let num2 = Math.ceil(x/y);
            //setCount(num2);
            return num2
        }
        console.log(num());
    ...
    const handleChangeSelect = (event) => {
        ...
        if(event.target.value ==='all'){
            ...
            let first = usuarios.filter(myFunction);
            function myFunction(value, index, array) {
                return value.tipo === "ADMINISTRADOR";
            }
            console.log(first);
            setUsers(first);
                    ...
                    <Box display='flex' justifyContent='center' alignItems='center' w='80vw' bg='gray.400'>
                        <Box display='flex' flexDirection='column' alignItems='center' w='70vw' h='80vh' bg='white' borderRadius='md' boxShadow='0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)' > 
                            ...
                            <Box display='flex' flexDirection='row' justifyContent='space-between' w='60vw' marginTop='3'>
                                <Busca />
                                <Select borderColor='Black' w='10vw' onChange={handleChangeSelect}>
                            ...
                            <Box display='flex' flexDirection='row' justifyContent='center' w='60vw' marginBottom='3' marginTop='3'>
                                <List objetarray={users}/>
                            </Box>
                            <Box display='flex' flexDirection='row' justifyContent='center'>
                                {/*<Pagination count={count} />*/}
                                <ThemeProvider theme={theme}>
                                    <Stack spacing={2}>
                                        <Pagination count={count} page={page} onChange={handleChangePagination} variant="outlined" shape="rounded" />
                                    </Stack>
                                </ThemeProvider>
                            </Box>      
                        </Box>
